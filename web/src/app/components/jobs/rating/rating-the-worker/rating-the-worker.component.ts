...
import { ApplicantReturn, RatingTheWorkerByTheEmployer } from '../../_models/applicant';
import { ApplicantsService } from '../../_services/applicants.service';

@Component({
  selector: 'app-rating-the-worker',
  templateUrl: './rating-the-worker.component.html',
  styleUrls: ['./rating-the-worker.component.css']
})
export class RatingTheWorkerComponent implements OnInit {
  ...
  ratingTheEmployerByTheWorker() {
    if (this.ratingForm.valid) {
      let ratingData = {} as RatingTheWorkerByTheEmployer;
      ratingData = Object.assign({}, this.ratingForm.value);
      this.applicantService.ratingTheWorkerByTheEmployer(ratingData).subscribe(
