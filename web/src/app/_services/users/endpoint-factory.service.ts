...
export class EndpointFactory {
  ...
  private readonly appVersion: string = '2.7.2';
  ...
  getLoginEndpoint<T>(userName: string, password: string): Observable<T> {
    ...
    return this.http.post<T>(this._authService.loginUrl, params, { headers: header });
  }
  ...
  protected getRequestHeaders(): { headers: HttpHeaders | { [header: string]: string | string[]; } } {
    const headers = new HttpHeaders({
      'Authorization': 'Bearer ' + localStorage.getItem(DbKeysService.ACCESS_TOKEN),
  ...
  protected handleError(error, continuation: () => Observable<any>) {
    if (error.status == 401) {
      ...
      //    if (refreshLoginError.status == 401 || (refreshLoginError.url && refreshLoginError.url.toLowerCase().includes(this._authService.loginUrl.toLowerCase()))) {
    ...
    if (error.url && error.url.toLowerCase().includes(this._authService.loginUrl.toLowerCase())) {
      //this.authService.reLogin();
